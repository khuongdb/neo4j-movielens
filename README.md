...
- Test performance between content-based and  collective-filtering with difference size of test data. 
- Change paramaters (kNN, number of common movie)
- Content based: diff technique for predicting (mean, median, mode)
- Collective-filtering: use bin of rating for prediction: 
  - AlgorithmL given target_user and target_movie:
    - find other users who rated the target_movie.
    - find common movies that target_user and u2 have rated. 
    - Calculate the similarity score. 
    - For the target movies - Divide the u2 ratings into bind: 
      [0..0.5] ratings -> [4.5..5.0] ratings. 
      meaning for target_movie: we will have 10 bins. (0 to 5 times 2) and we have k-neighbors. 
    - For each bin, we will count the number of similar u2. 
    for example: [0..0.5]: 3 u2. [4.0..4.5]: 5 u2 -> we will take the bin 4.0..4.5 -> we will take the mode of the bin. 
...
- Difference methods: mean, median, mode. 

### Collective-filtering:

- Collective filtering with non-normalize similarity score (all similarity score > 0)

- Collective filtering with normalized similarity score (we will have negative correlation)

- Collective filtering with binning technique. 

For each method, we can try diff parameters: number of k-Neighbors, number of common movies that we take into account (more conservative)

Table / figures. 

### Comparison and conclusion. 

- Runtime: conten-based < collective-filter < collective-filter-normalize < collective-filter-bin
- Accuracy (RMSE): content-based is best. 